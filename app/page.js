...
export default function Dashboard() {
  ...
  function statusFor(p) {
    if (p.quantity <= 0) return { label: 'Out of Stock', bg: 'bg-[var(--rust)]/10 text-[var(--rust)] border-[var(--rust)]/20', rail: 'bg-[var(--rust)]' };
    if (p.quantity <= (p.reorder_level || 10)) return { label: 'Low Stock', bg: 'bg-[var(--brass)]/10 text-[var(--brass)] border-[var(--brass)]/20 animate-pulse', rail: 'bg-[var(--brass)]' };
    return { label: 'In Stock', bg: 'bg-[var(--steel)]/10 text-[var(--steel)] border-[var(--steel)]/20', rail: 'bg-[var(--steel)]/30' };
  }
 ...
 function formatDate(dateStr) {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      {/* Header section with an industrial ledger style */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-[var(--ink)]/10 pb-5">
        ...
        <div className="bg-[var(--steel)]/5 px-4 py-2 border border-[var(--steel)]/10 h-fit w-fit rounded-sm shadow-inner">
          <span className="text-xs font-mono uppercase tracking-widest text-[var(--ink)]/70">
            Active SKUs: <b className="text-[var(--steel)] ml-1 font-bold text-sm">{products.length}</b>
          </span>
        </div>
      </div>
      ...
      {!loading && products.length === 0 && (
        <div className="border-2 border-dashed border-[var(--ink)]/20 bg-[var(--card)] p-12 text-center max-w-xl mx-auto shadow-[4px_4px_0px_0px_rgba(0,0,0,0.05)]">
          <p className="font-display font-extrabold text-xl uppercase tracking-wide text-[var(--ink)] mb-2">No stock on record</p>
